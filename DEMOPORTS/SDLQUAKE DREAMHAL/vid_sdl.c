...
/*Dreamhal template for porting games*/
  ...
  //  if (!(screen = SDL_SetVideoMode(vid.width, vid.height, 8, flags)))
    //    Sys_Error("VID: Couldn't set video mode: %s\n", SDL_GetError());
           screen=SDL_SetVideoMode(TXR_X,TXR_Y,8,SDL_HWSURFACE);
           SDL_DC_SetWindow(SCR_X,SCR_Y);
      // screen=SDL_SetVideoMode(vid.width, vid.height, 8,SDL_FULLSCREEN|SDL_HWSURFACE);
      //  screen=SDL_SetVideoMode(vid.width, vid.height, 8,SDL_FULLSCREEN|SDL_DOUBLEBUF|SDL_HWSURFACE);
    ...
    vid.aspect = ((float)vid.height / (float)vid.width) * (320.0 / 179.0);
    vid.numpages = 2;
