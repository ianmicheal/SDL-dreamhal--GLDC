...
float rSqrt1(float x) { return 1.f / sqrtf(x);  
 ...
 float  sqrt2(const float x)
{
  ...
  u.x = x;
  u.i = MATH_Fast_Sqrt - (u.i >> 1);  // gives initial guess y0
...
float VectorNormalize (vec3_t v)
{
	...
	length = MATH_Fast_Sqrt (length);		// FIXED 
...
//				numer, denom);
	...
	if (numer >= 0.0)
	{
		...
		x = PSEUDO_FLOOR(numer / denom);
		q = (int)x;
		r = (int)PSEUDO_FLOOR(numer - (x * denom));
	}
	else
	{
	//
...
// Ian micheal ported fasted GCD version to Dreamcast
 int GreatestCommonDivisor( int i1,  int i2)
{
    int shift;
    if (i1 == 0) return i2;
    if (i2 == 0) return i1;
    shift = __builtin_ctz(i1 | i2);
    i1 >>= __builtin_ctz(i1);
    do {
        i2 >>= __builtin_ctz(i2);
        if (i1 > i2) {
