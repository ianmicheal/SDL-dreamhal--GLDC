...
void * memmove_16bitt(void *dest, const void *src, size_t len);
void * memmove_32bitt(void *dest, const void *src, size_t len);
void * memmove_64bit(void *dest, const void *src, size_t len); // There is a way :)

void * memcpyasm (void *dest, const void *src, size_t len);
void * memcpy_16bitasm(void *dest, const void *src, size_t len);
void * memcpy_32bitasm(void *dest, const void *src, size_t len);
void * memcpy_32bit_16Bytesasm(void *dest, const void *src, size_t len); // 16 bytes, 4-byte alignmnet
void * memcpy_64bitasm(void *dest, const void *src, size_t len);
void * memcpy_64bit_32Bytesasm(void *dest, const void *src, size_t len); // 32 bytes, 8-byte alignment
// NOTE: Store Queues are better, though they require 32-byte alignment.

void * memsethal (void *dest, const uint8_t val, size_t len);
void * memset_16bitt(void *dest, const uint16_t val, size_t len);
void * memset_32bitt(void *dest, const uint32_t val, size_t len);
// NOTE: 64bit takes 32-bit input data and writes it two times simultaneously per write
void * memset_64bit_4BB(void *dest, const uint32_t val, size_t len);
// Write 4 or 8 bytes of zeroes at a time
void * memset_zeroes_32bitt(void *dest, size_t len);
void * memset_zeroes_64bitt(void *dest, size_t len);
